...
async function getDriver() {
    ...
    const prefs = { 'profile.default_content_setting_values.automatic_downloads': 1, 'download.prompt_for_download': false, 'download.directory_upgrade': true, 'safebrowsing.enabled': true }; 
...
async function startScreenshare(driver: WebDriver) {
    var totalTime=60000
    var clipTime=10000 // --> minimum 2-3 mins 
        ...
        const response = await driver.executeScript(`
            ...
            const combinedStream = new MediaStream([
                ...
                for(var i=0;i<arguments[0]/arguments[1];i++)
                {
                    const recordedChunks = await startRecording(combinedStream, arguments[1]); // always 10 seconds clips
...
async function main() {
    ...
    await openMeet(driver,'https://meet.google.com/ybi-eurv-efr');
